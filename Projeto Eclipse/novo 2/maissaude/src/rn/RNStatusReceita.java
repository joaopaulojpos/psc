...
public class RNStatusReceita {
	...
	private void validarDespachoPerfil(StatusReceita statusReceita)throws ValidacaoException{		
		...
		atendente = statusReceita.getUsuarioAlteracao().getAtendente();//dao.getDAOAtendente().pesquisarUserId(statusReceita.getUsuarioAlteracao().getIdUsuario());
			if(atendente.isFarmaceutico()){
				
			}else
				if(statusReceita.getReceita().getIsControlada()){
